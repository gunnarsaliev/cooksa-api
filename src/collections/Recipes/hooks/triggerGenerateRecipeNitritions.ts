...
async function calculateTotalRecipeWeight(
  ...
  for (const ingredient of ingredients) {
    ...
    const ingredientId =
      typeof ingredient.ingredient === 'number' ? ingredient.ingredient.toString() : undefined
 ...
 */
function calculateNutritionPer100g(
...
function validateAndLogNutritionCalculation(
  ...
  const expectedCalories =
    nutritionPer100g.protein * 4 + nutritionPer100g.fat * 9 + nutritionPer100g.carbohydrates * 4
...
export const triggerGenerateRecipeNitritions: CollectionAfterChangeHook<Recipe> = async ({
  doc,
  req,
  operation,
}) => {
  ...
  if (req.locale && req.locale !== 'all') {
    console.log(
      `⏭️  Skipping recipe nutrition generation - not English locale (current: ${req.locale})`,
    )
    return doc
  }
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      if (existing.docs.length > 0 && !isEditingPublishedRecipe) {
        console.log(
          '❌ Skipping - recipe nutrition already exists and this is not an edit of a published recipe',
        )
        return
      }
      ...
      for (const recipeIngredient of doc.ingredients) {
        ...
        try {
          ...
          if (ingredientNutrition.docs.length === 0) {
            console.log(
              `⚠️ No nutritional data found for ingredient ID: ${recipeIngredient.ingredient}`,
            )
            skippedIngredients++
            continue
          }
          ...
          const weightInGrams = await convertToGrams(
            amount,
            unit,
          ...
          console.error(
            `❌ Error processing ingredient ${recipeIngredient.ingredient || 'unknown'}:`,
            error,
          )
          skippedIngredients++
        }
      ...
      console.log(
        `📊 Nutrition calculation complete: ${processedIngredients} ingredients processed, ${skippedIngredients} skipped`,
      )
      ...
      validateAndLogNutritionCalculation(
        totalNutrition,
        nutritionPer100g,
        totalWeight,
        processedIngredients,
      ...
      if (isFirstTimePublishing) {
        ...
        req.payload.logger.info(
          `Recipe nutrition calculated for recipe: ${doc.name} (ID: ${doc.id})`,
        )
      } else if (isEditingPublishedRecipe) {
      ...
      req.payload.logger.error(
        `Error in triggerRecipeCalculator: ${error instanceof Error ? error.message : String(error)}`,
      )
    }
